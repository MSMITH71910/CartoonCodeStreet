...
/**
 * ProceduralMusicLayer Component
 * 
 * This component generates completely different procedural music for each
 * activity using the Web Audio API directly, bypassing the MP3 file issues.
 * 
 * It runs independently of the rest of the audio system and plays unique
 * music for each mini-game and interactive object.
 */
const ProceduralMusicLayer: React.FC = () => {
  // References to track audio contexts and cleanup functions
  const audioContextRef = useRef<AudioContext | null>(null);
  const cleanupFuncsRef = useRef<{[key: string]: (() => void) | null}>({
    background: null,
    chess: null,
    fountain: null,
    seesaw: null
  ...
  const interactionType = useInteraction(state => state.interactionType);
  
  // Get audio mute state
  const isMuted = useAudio(state => state.isMuted);
  const isMusicMuted = useAudio(state => state.isMusicMuted);
  
  // Determine which music type should be playing based on interaction
  const determineMusicType = () => {
    // Default to background music
    let musicType = 'background';
    
    // Map interaction types to music types
    ...
    console.log(`PROCEDURAL MUSIC: Determined music type "${musicType}" for interaction "${interactionType}"`);
    return musicType;
  };
  
  // Effect for managing the audio context lifecycle
  useEffect(() => {
    ...
    if (!audioContextRef.current) {
      try {
        ...
        console.log('PROCEDURAL MUSIC: Audio context created');
      } catch (err) {
        console.error('PROCEDURAL MUSIC: Failed to create audio context:', err);
        return;
      }
    }
    ...
    return () => {
      // Clean up all audio
      Object.values(cleanupFuncsRef.current).forEach(cleanup => {
        if (cleanup) {
          cleanup();
        }
      });
      ...
      if (audioContextRef.current) {
        audioContextRef.current.close().catch(err => {
          console.error('PROCEDURAL MUSIC: Error closing audio context:', err);
        });
        audioContextRef.current = null;
      }
      
      console.log('PROCEDURAL MUSIC: Cleaned up all audio');
    };
  }, []);
  
  // Effect for changing music based on interaction type
  useEffect(() => {
    // Skip if audio is muted or context not available
    if (isMuted || isMusicMuted || !audioContextRef.current) {
      console.log('PROCEDURAL MUSIC: Audio is muted or context unavailable, not generating');
      return;
    }
    
    // Get the currently appropriate music type
    const musicType = determineMusicType();
    console.log(`PROCEDURAL MUSIC: Switching to ${musicType} music`);
    
    // First stop all currently playing music
    Object.entries(cleanupFuncsRef.current).forEach(([type, cleanup]) => {
      if (type !== musicType && cleanup) {
        cleanup();
        cleanupFuncsRef.current[type] = null;
        console.log(`PROCEDURAL MUSIC: Stopped ${type} music`);
      }
    });
    
    // Start the new music if not already playing
    if (!cleanupFuncsRef.current[musicType]) {
      console.log(`PROCEDURAL MUSIC: Starting ${musicType} music`);
      
      let cleanup: (() => void) | null = null;
      
      switch (musicType) {
        case 'background':
          cleanup = generateBackgroundMusic(audioContextRef.current);
          break;
        case 'chess':
          cleanup = generateBoardGameMusic(audioContextRef.current);
          break;
        case 'fountain':
          cleanup = generateFountainMusic(audioContextRef.current);
          break;
        case 'seesaw':
          cleanup = generatePlaygroundMusic(audioContextRef.current);
          break;
      }
      
      cleanupFuncsRef.current[musicType] = cleanup;
    }
    
  }, [interactionType, isMuted, isMusicMuted]);
  
  // Effect for handling mute state changes
  useEffect(() => {
    if (isMuted || isMusicMuted) {
      // Stop all music when muted
      Object.entries(cleanupFuncsRef.current).forEach(([type, cleanup]) => {
        if (cleanup) {
          cleanup();
          cleanupFuncsRef.current[type] = null;
          console.log(`PROCEDURAL MUSIC: Stopped ${type} music due to mute`);
        }
      });
    } else {
      // Restart music for current interaction when unmuted
      const musicType = determineMusicType();
      
      if (!cleanupFuncsRef.current[musicType] && audioContextRef.current) {
        console.log(`PROCEDURAL MUSIC: Restarting ${musicType} music after unmute`);
...
function generateBackgroundMusic(audioContext: AudioContext): () => void {
  console.log('PROCEDURAL MUSIC: Generating background music');
...
function generateBoardGameMusic(audioContext: AudioContext): () => void {
  console.log('PROCEDURAL MUSIC: Generating board game music');
...
function generateFountainMusic(audioContext: AudioContext): () => void {
  console.log('PROCEDURAL MUSIC: Generating fountain music');
...
function generatePlaygroundMusic(audioContext: AudioContext): () => void {
  console.log('PROCEDURAL MUSIC: Generating playground music');
...
export default ProceduralMusicLayer;