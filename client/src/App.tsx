...
// Direct access to audio functionality for mini-games
declare global {
  interface Window {
    playMiniGameMusic: () => void;
    playBackgroundMusic: () => void;
  }
}

// Global helper for playing mini-game (chess) music
window.playMiniGameMusic = function() {
  console.log("DIRECT AUDIO: Global mini-game music function called");
  const audioStore = useAudio.getState();
  
  if (!audioStore.chessMusicOrSimilar) {
    console.error("DIRECT AUDIO: Chess music not available in store");
    return;
  }
  ...
  try {
    ...
    // Play chess music for mini-games
    const chessMusic = audioStore.chessMusicOrSimilar;
    chessMusic.currentTime = 0;
    chessMusic.volume = 0.4; 
    
    // Update store state
    useAudio.setState({
      currentActivityMusic: chessMusic,
      currentTrack: "chess" 
    });
    
    // Play without Promise chain
    chessMusic.play();
    
    console.log("DIRECT AUDIO: Mini-game music started playing");
  } catch (e) {
    console.error("DIRECT AUDIO: Error playing mini-game music:", e);
  }
};

// Global helper for playing background music
window.playBackgroundMusic = function() {
  console.log("DIRECT AUDIO: Global background music function called");
  const audioStore = useAudio.getState();
  
  if (!audioStore.backgroundMusic) {
    console.error("DIRECT AUDIO: Background music not available in store");
    return;
  }
  ...
  try {
    ...
    const bgMusic = audioStore.backgroundMusic;
    ...
    console.log("DIRECT AUDIO: Background music started playing");
  } catch (e) {
    console.error("DIRECT AUDIO: Error playing background music:", e);
  }
};
